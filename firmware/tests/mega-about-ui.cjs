...
const repo = 'https://github.com/mfoster978/openwrt-zbtlink-zbt-z8803be-dual-modem-build';
const fixture = `
window.calls = [];
window._ = s => s;
...
(async () => {
  ...
  try {
    ...
    assert.equal(await page.locator('h1').innerText(), 'About Mega Firmware');
    assert.match(await page.locator('.zma-hero-title').innerText(), /ZBT-Z8803BE[\s\S]*Mega Firmware/);
