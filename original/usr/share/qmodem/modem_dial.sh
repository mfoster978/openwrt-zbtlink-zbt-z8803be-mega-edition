...
set_if()
{
    ...
    if [ -x /usr/bin/quectel-CM-M ] && [ "$driver" = "qmi" ]; then proto="none"; protov6="none"; fi
...
qmi_dial()
{
    ...
    while true; do
        ...
        $cmd_line &
        qmi_pid="$!"
        echo "$qmi_pid" > "${MODEM_RUNDIR}/${modem_config}_dir/$modem_config.pid"
        m_debug "pid: $qmi_pid"
        mtu_guard_pid=""
        if [ -x /usr/sbin/qmi-mtu-guard ] && [ -n "$qmi_if" ]; then
            /usr/sbin/qmi-mtu-guard "$qmi_if" "$qmi_pid" 1500 &
            mtu_guard_pid="$!"
        fi
        wait "$qmi_pid"
        [ -n "$mtu_guard_pid" ] && kill "$mtu_guard_pid" 2>/dev/null
        [ -n "$mtu_guard_pid" ] && wait "$mtu_guard_pid" 2>/dev/null
